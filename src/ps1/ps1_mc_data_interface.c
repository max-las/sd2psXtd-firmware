...
void __time_critical_func(ps1_mc_data_interface_start_dma)(uint32_t page) {
    ...
    psram_read_dma(page * PS1_SECTOR_SIZE, card, PS1_SECTOR_SIZE, ps1_mc_data_interface_rx_done);
}
#endif
...
#if WITH_PSRAM
    card[address%PS1_SECTOR_SIZE] = byte;
#else
    card[address] = byte;
#endif
    write_occured = true;
...
#if WITH_PSRAM
    psram_wait_for_dma();
    psram_write_dma(page * PS1_SECTOR_SIZE, card, PS1_SECTOR_SIZE, NULL);
    psram_wait_for_dma();
#endif
    ps1_dirty_mark(page);
    ps1_dirty_unlock();
...
#if WITH_PSRAM
    while (dma_in_progress && psram_read_dma_remaining() >= (PS1_SECTOR_SIZE - offset)) {};
#endif
}
...
void ps1_mc_data_interface_card_changed(void) {
    ...
    for (int i = 0; i < 1024; i++) {
        if (ps1_cardman_read_sector(i, &card[i*PS1_SECTOR_SIZE]) < 0)
